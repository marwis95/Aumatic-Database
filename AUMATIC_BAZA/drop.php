...
if (($prawa == "vip") || ($prawa == "admin")){

if($_GET["usun"]) {
	//echo "Usuwanie id ".$_GET['usun'];
	
	echo "<script>";
	echo "alert('Program zostal usuniety pomyslnie');";
	echo "</script>";
	
	$zapytanie="delete from programy where id='" . $_GET['usun'] . "'"; //query
	mysql_query($zapytanie);  // result
	
	//$zapytanie_plik="select * from programy where id='" . $_GET['usun'] . "'"; //query
	//$wykonaj_plik = mysql_query($zapytanie_plik);  // result
	//$wiersz_plik = mysql_fetch_array($wykonaj_plik);
	//echo $wiersz_plik['pobierz'];
	
	//echo "unlink('nazwa_pliku_2.rar')";
}
...
echo "Nie znalaziono takiego programu.<br>Zmień kryteria i wyszukaj ponownie.";
}
...
echo "<a href='drop.php?usun=".$wiersz['id']."'>-->USUŃ TEN PROGRAM<--</a>";
